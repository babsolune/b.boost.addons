...
<section id="content">
	<header>
		<h1>Package <a href="package-IO.php">IO</a>\Template</h1>
	</header>
	<article>
		<header><h2>Package <a href="package-IO.php">IO</a>\Template</h2></header>
		<div class="content">
			<div class="responsive-table">
				<table class="summary" id="packages">
					<caption>Packages summary</caption>
					<tr>
						<td class="name"><a href="package-IO.Template.data.php">IO\Template\data</a></td>
					</tr>
					<tr>
						<td class="name"><a href="package-IO.Template.loader.php">IO\Template\loader</a></td>
					</tr>
					<tr>
						<td class="name"><a href="package-IO.Template.parser.php">IO\Template\parser</a></td>
					</tr>
					<tr>
						<td class="name"><a href="package-IO.Template.renderer.php">IO\Template\renderer</a></td>
					</tr>
				</table>
			</div>
...
<tr>
	<td class="name"><a href="class-AbstractTemplate.php">AbstractTemplate</a></td>
	<td class="ellipsis"><p>This class is a default implementation of the Template interface using a TemplateLoader,
a TemplateData and a TemplateParser.</p></td>
</tr>
<tr>
	<td class="name"><a href="class-FileTemplate.php">FileTemplate</a></td>
	<td class="ellipsis"><p>This class allows you to handle a template file.
Your template files should have the .tpl extension.
To be more efficient, this class uses a cache and parses each file only once.</p>

<h4>File identifiers</h4>

<p>The web site can have several themes whose files aren't in the same folders. When you load a file, you just have to load the generic file and the right template file will
be loaded dynamically.</p>

<h5>Kernel template file</h5>

<p>When you want to load a kernel template file, the path you must indicate is only the name of the file, for example header.tpl loads /template/your_theme/header.tpl and
if it doesn't exist, it will load /template/default/header.tpl.</p>

<h5>Module template file</h5>

<p>When you want to load a module template file, you must indicate the name of you module and then the name of the file like this: module/file.tpl which will load the
/module/templates/file.tpl. If the user themes redefines the file.tpl file for the module module, the file templates/your_theme/modules/module/file.tpl will be loaded.</p>

<h5>Menu template file</h5>

<p>To load a file of a menu, use this kind of path: menus/my_menu/file.tpl which will load the /menus/my_menu/templates/file.tpl file.</p>

<h5>Framework template file</h5>

<p>To load a framework file, use a path like this: framework/package/file.tpl which will load /templates/your_theme/framework/package/file.tpl if the theme overrides it,
otherwise /templates/default/framework/package/file.tpl will be used.</p></td>
</tr>
<tr>
	<td class="name"><a href="class-I18NMessages.php">I18NMessages</a></td>
	<td class="ellipsis"></td>
</tr>
<tr>
	<td class="name"><a href="class-StringTemplate.php">StringTemplate</a></td>
	<td class="ellipsis"><p>This class enables you to handle a template whose input is not a file but directly a string.
...
a big efficiency. It will be the case for instance when you know that a string template will be used only once a month.</p></td>
</tr>
<tr>
	<td class="name"><a href="class-TemplateFunctions.php">TemplateFunctions</a></td>
	<td class="ellipsis"></td>
</tr>
	</table>
</div>
...
<tr>
	<td class="name"><a href="class-Template.php">Template</a></td>
	<td class="ellipsis"><p>This class represents a PHPBoost template. Templates are used to generate text which
...
PHPBoost's template engine is home made and has its own syntax which is the described below:</p>

<h4>Variable assignation</h4>

<h5>Simple variables</h5>

<p>A simple variable is accessible with the {NAME} syntax where NAME is its template name. If the variable is not assigned, nothing will be displayed (no error message).
Simple variables are assigned by the assign_vars() method.</p>

<h4>Lang variables</h4>

<p>To speed the development up, you can avoid setting all the localized variables to use. You just have to associate one or more lang map
(lang_identifier => localized lang) with the <code><a href="class-Template.php#_add_lang">Template::add_lang()</a></code> method. In the template, you just have to
add the 'L_' prefix before the variable name and it will be searched in the langs map.</p>

<h5>Loops</h5>

<p>You can make some loops to repeat a pattern, those loops can be nested. A loop has a name (name) and each iteration contains some variables, for example, the variable VAR.
<br />&#35; START name #
...
<br />&#35; END loop1.loop2 #
<br />&#35; END loop1 #
To assign the variable, see the assign_block_vars() method which creates one iteration.</p>

<h5>Conditions</h5>

<p>When you want to display something only in particular case, you can use some condition tests.
<br />&#35; IF C_MY_TEST #
<br />This text will be displayed only if the C_MY_TEST variable is true
<br />&#35; ENDIF #
You can nest some conditions.</p>

<h3>Nesting templates</h3>

<p>You can embed a template in another one. For that, you have to use the INCLUDE instruction like that: # INCLUDE template #
...
You also can include templates in a loop, for that you have to place them in the third parameter of the asssign_block_vars() method.</p>

<h4>Variables using</h4>

<p>By default, when you call the variable by it's name, it'll be returnes as it was assigned. But you can do little implicit treatments of variables
directly in the template. Here are the prefixes to use before the variable names:</p>

<ul>
<li><E_: the variable will be searched without its prefix and will be escaped using <pre><span class="php-keyword2">htmlspecialchars</span>()</pre></li>
<li><J_: the variable will be searched without its prefix and will be escaped using <pre>TextHelper::to_js_string()</pre></li>
<li><L_: the variable will be searched without its prefix in every languages maps registered using <pre>Template-&gt;add_lang(<span class="php-var">$language</span>)</pre></li>
<li><EL_: the variable will be searched without its prefix like languages variables and will be escaped using <pre><span class="php-keyword2">htmlspecialchars</span>()</pre></li>
<li><JL_: the variable will be searched without its prefix like languages variables and will be escaped using <pre>TextHelper::to_js_string()</pre></li>
</ul></td>
</tr>
<tr>
	<td class="name"><a href="class-View.php">View</a></td>
	<td class="ellipsis"></td>
</tr>
	</table>
</div>
...
<tr>
	<td class="name"><a href="class-FileTemplateRenderingException.php">FileTemplateRenderingException</a></td>
	<td class="ellipsis"></td>
</tr>
<tr>
	<td class="name"><a href="class-InvalidTemplateFunctionCallException.php">InvalidTemplateFunctionCallException</a></td>
	<td class="ellipsis"></td>
</tr>
<tr>
	<td class="name"><a href="class-TemplateRenderingException.php">TemplateRenderingException</a></td>
	<td class="ellipsis"></td>
</tr>
	</table>
</div>
